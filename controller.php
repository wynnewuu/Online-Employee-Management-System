...
if (isset($_POST['submit'])) { // Check if post action
    ...
    if ($errors) {
        ...
        $sql = "INSERT INTO EMPLOYEE (ssn, bdate, fname, lname, address, salary, dno) VALUES (:ssn, :bdate, :fname, :lname, :address, :salary, :dno)";
        // Insert data from post action
        $result = $db->insert($sql, $conditions);
    ...
    // Insert Sql statment with interpolated variables `:variable`
    $sql = "SELECT ssn, bdate, fname, lname, address, salary, dno 
    FROM EMPLOYEE";
